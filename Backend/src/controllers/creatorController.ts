...
import { getCreatorProfile, getCreatorsLeaderboard, getTopFans, registerCreator } from "../services/creatorService.js";
...
export const RegisterCreator=async (req: Request, res: Response) => {
    try{
        ...
        const creator=await registerCreator(userId, displayName, coinAddress, pfp);
        if(creator){
...
export const GetCreatorProfile=async (req: Request, res: Response) => {
    try{
        ...
        const creator=await getCreatorProfile(Number(creatorId));
        if(creator){
...
export const GetTopFans=async (req: Request, res: Response) => {
    try{
        ...
        const topFans=await getTopFans(Number(creatorId));
        if(topFans){
