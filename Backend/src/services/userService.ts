...
export const CalculatePendingRewards = async () => {
  try {
    const result = await db
      ...
      .innerJoin(
        creators,
        sql`${String(rewards.coinAddress).toLowerCase()} = ${String(
          creators.coinAddress
        ).toLowerCase()}`
      )
      .where(sql`${rewards.claimed} = false`)
...
export const distributeRewardsCron = async () => {
  console.log(
    "Starting reward distribution cron job at",
  ...
  try {
    ...
    const aggregatedRewards = pendingRewards.reduce(
      (
        acc: Record<
          string,
          {
      ...
      while (attempt < Config.maxRetries && !success) {
        try {
          ...
          console.log(
            `Attempt ${
              attempt + 1
            }: Distributing ${totalAmount} to ${userAddress} for creatorId ${
              reward.creatorId
            }`
          );
          ...
          if (idsArray.length > 0) {
            ...
            console.log(
              `Marked rewards with IDs ${idsArray.join(
                ", "
              )} as claimed for user ${userAddress} and coin ${coinAddress}`
            );
          }
          ...
          attempt++;
          console.error(
            `Attempt ${attempt} failed for ${userAddress} (coin: ${coinAddress}):`,
            error
          );
          if (attempt < Config.maxRetries) {
            ...
            console.error(
              `Max retries reached for ${userAddress} (coin: ${coinAddress}). Skipping.`
            );
          }
        }
      }
